...
# Function to match synonyms from the category dictionary
def match_category(transaction, categories):
    transaction = transaction.lower()  # Convert the transaction description to lowercase
    for category, synonyms in categories.items():
        for synonym in synonyms:
            if synonym.lower() in transaction:
                return category  # Return the category if a match is found
    return transaction  # If no match, return the original transaction description

file_path = 'C:/Users/naman/Documents/Data Science/RA/data/updated_dataset.csv'  
df = pd.read_csv(file_path)
print(df.head())

# Drop unnecessary columns (use only transaction_type and category)
df = df[['transaction_type', 'category']]

# Preprocess text by matching the transaction_type column with category synonyms
# Assuming `match_category` is a function you have defined
# This step is needed to align transaction_type with a category.
df['transaction_type'] = df['transaction_type'].apply(lambda x: match_category(x, categories))

# Splitting data into training and testing sets
X = df['transaction_type']  # Independent variable (transaction description)
y = df['category']          # Dependent variable (category label)
...
X_test_tfidf = vectorizer.transform(X_test)        # Only transform on testing data
...
# Now let's classify a new transaction description using the trained model
new_transaction = ["Today, I earned rewards from my credit card."]  # Example new input
new_transaction_tfidf = vectorizer.transform(new_transaction)       # Transform it with the same vectorizer
predicted_category = nb_model.predict(new_transaction_tfidf)        # Predict the category
