...
# Function to classify the text based on categories dictionary
def classify_text(text, categories):
    ...
    for category, phrases in categories.items():
        ...
        if matched_category:
            break
    return matched_category or 'Uncategorized'

def classify_and_save_output(transaction_details, user_input, categories, filename):
    transaction_details['user_input'] = user_input
    
    # Classify the transaction
    classification = classify_text(user_input, categories)
    transaction_details['category'] = classification
    
    # Save to CSV
    fieldnames = list(transaction_details.keys())
    save_to_csv(filename, fieldnames, transaction_details)
...
#     # Initialize the classification dictionary with subcategories
#     classification = {category: 0 for category in categories.keys()}
...
#                 classification['notes'].append(phrase.lower())
    
#     # Join the notes into a single string
#     classification['notes'] = ', '.join(classification['notes'])

#     # Determine if any of the subcategories belong to the main categories
#     classification['neither_expense_nor_income'] = int(any(
#         classification[cat] for cat in [
#             'put_money_in_bank', 'took_money_out_of_bank', 'got_a_loan', 
#             'received_a_line_of_credit', 'took_out_a_mortgage', 'paid_a_loan', 
#             'bought_a_machine', 'bought_a_vehicle', 'bought_real_estate', 
#             'bought_furniture', 'bought_a_copier', 'bought_a_computer', 
#             'bought_tools_or_equipment', 'paid_a_personal_expense'
#         ]
#     ))

#     classification['income'] = int(any(
#         classification[cat] for cat in [
#             'sold_asset', 'earned_interest', 'received_dividend', 
#             'received_credit_card_rewards', 'got_refund_of_expense', 
#             'got_other_income', 'got_tax_refund', 'got_partnership_income', 
#             'sale_of_product_income', 'sale_of_service_income'
#         ]
#     ))

#     classification['expense'] = int(any(
#         classification[cat] for cat in [
#             'inventory', 'asset', 'loans_to_others', 'payments_to_deposit', 
#             'uncategorized_asset', 'buildings', 'land', 'office_equipment', 
#             'computers_tablets', 'copiers', 'custom_software_app', 'furniture', 
#             'phones', 'photo_video_equipment', 'tools_machinery_equipment', 
#             'vehicles', 'customer_prepayments', 'sales_tax_payments', 
#             'short_term_business_loans_payments_made', 
#             'long_term_business_loans_payments_made', 'mortgages_payments_made', 
#             'federal_estimated_taxes_paid', 'personal_expenses_federal_taxes', 
#             'owner_retirement_plans', 'personal_expenses', 'state_taxes', 
#             'personal_healthcare', 'personal_healthcare_health_insurance_premiums', 
#             'personal_healthcare_hsa_contributions', 'state_estimated_taxes', 
#             'refunds_to_customers', 'cost_of_goods_sold', 'equipment_rental', 
#             'subcontractor_expenses', 'supplies_and_materials', 
#             'advertising_and_marketing', 'listing_fees', 
#             'social_media_advertising_and_marketing', 'website_ads', 
#             'building_and_property_rent', 'business_licenses', 
#             'commissions_and_fees', 'contract_labor', 'contributions_to_charities', 
#             'employee_benefits', 'employee_retirement_plans', 
#             'employee_benefits_group_term_life_insurance', 
#             'employee_benefits_health_and_accident_plans', 
#             'workers_compensation_insurance', 'entertainment_with_clients', 
#             'general_business_expenses', 'bad_debt_general', 
#             'bank_fees_and_service_charges', 'continuing_education', 
#             'memberships_and_subscriptions', 'uniforms', 'liability_insurance', 
#             'property_insurance', 'rental_insurance', 'business_loan_interest', 
#             'credit_card_interest', 'mortgage_interest', 'accounting_fees', 
#             'legal_fees', 'meals_with_clients', 'travel_meals', 
#             'merchant_account_fees', 'office_supplies', 
#             'printing_and_photocopying', 'shipping_and_postage', 
#             'small_tools_and_equipment', 'software_and_apps', 'wages', 
#             'repairs_and_maintenance', 'supplies_and_materials', 'payroll_taxes', 
#             'property_taxes', 'airfare', 'hotels', 'taxis_or_shared_rides', 
#             'vehicle_rental', 'uncategorized_expense', 
#             'disposal_waste_fees_utilities', 'electricity_utilities', 
#             'heating_cooling_utilities', 'internet_tv_services', 
#             'phone_service', 'utilities_water_sewer', 'depreciation_home_office', 
#             'home_office_homeowner_rental_insurance', 'home_office_home_utilities', 
#             'home_office_mortgage_interest', 'home_office_property_taxes', 
#             'home_office_rent', 'home_office_repairs_maintenance', 
#             'vehicle_expenses', 'parking_tolls'
#         ]
#     ))
...
#     transaction_details['recipient'] = ask_question("Ok, whom did you pay?")
#     transaction_details['category'] = ask_question("Great, what was it for?")
#     transaction_details['payment_method'] = ask_question("Okay, what was the method of payment?")

#     # Classify the transaction description
#     classification = classify_text(transaction_details['category'], categories)
#     transaction_details.update(classification)
    
#     save_to_csv('expenses.csv', ['type', 'amount', 'recipient', 'category', 'payment_method', *classification.keys()], transaction_details)
...
#         transaction_details['payer'] = ask_question("Who paid you?")
#         transaction_details['category'] = ask_question("Great. Was it a sale of a product or service or for something else?")

#         # Classify the transaction description
#         classification = classify_text(transaction_details['category'], categories)
#         transaction_details.update(classification)

#         if "product" in transaction_details['category']:
#             save_to_csv('income_product.csv', ['type', 'amount', 'payment_method', 'payer', 'category', *classification.keys()], transaction_details)
#         elif "service" in transaction_details['category']:
#             save_to_csv('income_service.csv', ['type', 'amount', 'payment_method', 'payer', 'category', *classification.keys()], transaction_details)
#         else:
#             save_to_csv('income_miscellaneous.csv', ['type', 'amount', 'payment_method', 'payer', 'category', *classification.keys()], transaction_details)
        
#     elif "no" in response:
#         transaction_details['type'] = 'other'
#         transaction_details['category'] = ask_question("Okay, then please describe the transaction.")
#         transaction_details['involved_person'] = ask_question("Who else was involved?")
...
#         save_to_csv('other_transactions.csv', ['type', 'category', 'involved_person', 'amount', *classification.keys()], transaction_details)
...
# Ask the user to describe the transaction
user_input = ask_question("Please describe the transaction.")

# Classify the user input and save it
classify_and_save_output(transaction_details, user_input, categories, 'transactions.csv')
