...
# Add more involvement descriptions for variety
involvement_templates = [
    "I dealt with {}", "The transaction involved {}", "The money was sent to {}", 
    "{} was the recipient", "I conducted business with {}", "{} was involved in the transaction"
]

involvements = ["my friend", "my colleague", "a company", "a store", "a vendor", "ABC Corp", "XYZ Ltd", "ACME Corp"]
...
def generate_transaction_detail():
    amount = random.choice(amount_descriptions).format(
        random.choice(["100 dollars", "500 dollars", "1500 dollars", "2000 dollars", "2500 dollars"]), 
        random.choice(involvements)
    )
    
    # Generate a variety of involvement descriptions
    involvement = random.choice(involvement_templates).format(random.choice(involvements))
...
# Function to classify the transaction based on the categories provided
def classify_text(text, categories):
    ...
    # Iterate through each sub-category and its phrases
    ...
    # If no specific sub-category is found, classify as 'other'
...
def generate_dataset(num_rows, filename):
    with open(filename, mode='w', newline='') as file:
        ...
        writer.writerow(['amount', 'involvement', 'payment_method', 'transaction_type', 'category'])

        for _ in range(num_rows):
            transaction_detail = generate_transaction_detail()
            
            # Classify the transaction type based on the sub-categories
            category = classify_text(transaction_detail['transaction_type'], categories)
...
generate_dataset(100000, './data/dataset_with_subcategories.csv')

print("Dataset with subcategories generated and saved to 'dataset_with_subcategories.csv'")